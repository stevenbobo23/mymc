...
function spawnMobs(){
  if(gameMode==='shooter')return; // 竞技场无生物
...
function tryShootGun(){
  const held=heldItemId();const it=held?ITEMS[held]:null;
  if(!it||it.type!=='gun')return false;
...
function shooterRespawn(){
  ...
  player.pos.set(sp[0]+0.5,ARENA_GROUND+1,sp[1]+0.5);
  player.peakY=player.pos.y;player.mounted=null;
  for(const k in SHOOTER.ammo)delete SHOOTER.ammo[k]; // 补满弹
  if(inv.hot[6]&&inv.hot[6].id===4)inv.hot[6].count=200; // 补满掩体方块
  SHOOTER.reloading=false;
...
function shooterInit(){
  shooterReset();
  inv.hot=[{id:I.pistol,count:1},{id:I.smg,count:1},{id:I.shotgun,count:1},{id:I.sniper,count:1},{id:I.grenade,count:5},{id:I.mine,count:999},{id:4,count:200},null,null]; // 第7格=掩体方块（圆石×200）
